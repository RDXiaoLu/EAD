...
# from projects.mmdet3d_plugin.VAD.modules.motion.instance_queue import InstanceQueue
from projects.mmdet3d_plugin.VAD.modules.temporal_self_attention import TemporalSelfAttention  # 轻量 GNN
from projects.mmdet3d_plugin.VAD.attention import gen_sineembed_for_position
...
class GraphAttentionLayer(nn.Module):  
    def __init__(self, input_dim, output_dim, alpha=0.2):  
        ...
        # Linear transformation for the features  
        self.W = nn.Linear(input_dim, output_dim, bias=False)  # 权重矩阵  
        # 注意力共享的线性变换（self-attention）  
        self.a = nn.Linear(2 * output_dim, 1, bias=False)  # 注意力机制权重计算  
    ...
    def forward(self, x, edge_index):  
        ...
        # 计算注意力系数  
        # 通过 edge_index 生成注意力边 (i, j)  
        row, col = edge_index  
        # 组合 (i, j) 节点的特征  
        e = self.a(torch.cat([h[row], h[col]], dim=1))  # edge attention  
        
        # 使用 LeakyReLU 激活函数  
        e = F.leaky_relu(e, negative_slope=self.alpha)  
        
        # 计算注意力权重  
        attention = F.softmax(e, dim=0)  # softmax 归一化  
        
        # 聚合  
        h_prime = torch.zeros_like(h)  
        for i in range(N):  
            # 选择所有邻居节点的特征，通过索引和权重聚合  
            h_prime[i] = torch.sum(attention[edge_index[0] == i].view(-1, 1) * h[edge_index[1][edge_index[0] == i]], dim=0)  
...
class InstanceQueue:
    ...
    def push(self, feature, reference):
        # Push new feature, reference, instance ID, and confidence
        self.queue.append((feature, reference))
    ...
    def get(self):
        if not self.queue:
            return None, None
        features, references = zip(*self.queue)  # Unzip features and references
        return torch.stack(features), torch.stack(references)  # Return stacked tensors
...
class TemporalGNN(nn.Module):
    def __init__(self, embed_dims):
        ...
        self.layer = nn.Linear(embed_dims, embed_dims)  # Example layer

    def forward(self, query, key, value):
        # Sample GNN logic
        out = self.layer(query) + key  # Apply some operation
        return out
...
class GNNModule(nn.Module):  
    ...
    def aggregate(self, features):  
        # 特征整合过程  
        # features shape: [N, B, A, D]  
        N, B, A, D = features.shape  
        device = features.device  

        # 将 features 展平并转移到设备  
        features = features.view(-1, D).to(device)  

        # print(f"Flattened features shape: {features.shape}")  # 调试信息  

        # 创建边索引  
        edge_index = self.create_edges(N * B, device)  

        # 前向传播通过图卷积  
        out = self.forward(features, edge_index)  

        # print(f"Output shape from GNN: {out.shape}")  # 调试信息  

        # 将输出特征 reshape 为 (B, A, N, output_dim)  
        output_dim = out.shape[1]  
        out = out.view(B, A, N, output_dim)  
        
        # 使用全局平均池化来合并时间步维度  
        pooled_out = out.mean(dim=2)  

        return pooled_out  # 返回形状为 [B, A, output_dim]  

    def create_edges(self, num_nodes, device):  
        if num_nodes <= 1:  
            return torch.tensor([], dtype=torch.long, device=device).view(2, 0)  

        # 使用 PyTorch Geometric 提供的组合功能创建完全连接的边  
        edge_index = torch.combinations(torch.arange(num_nodes, device=device), r=2)  
        edge_index = edge_index.view(-1, 2).t()  # 转置为 [2, num_edges]  
...
#         self.lstm = nn.LSTM(input_dim, output_dim, num_layers=num_layers, batch_first=True)
...
#         # Reshape to (B, 300, N*256) for LSTM input
#         device = x.device  
#         x = x.permute(1, 2, 0, 3)  # Shape: (B, 300, N*256)
#         x = x.mean(dim=2)
#         print(f"x shape: {x.shape}")
       
#         print(f"x linear shape: {x.shape}")
#         # Forward through LSTM
#         output, (hn, cn) = self.lstm(x)  # output shape: (B, 300, output_dim)

#         return output  # shape: (B, 300, output_dim)
...
class VADHead(DETRHead):
    ...
    def __init__(self,
                ...
                loss_map_cls=dict(
                 ...
                 loss_plan_col=dict(type='PlanCollisionLoss', loss_weight=0.1),
                 loss_plan_dir=dict(type='PlanMapDirectionLoss', loss_weight=0.1),
        ...
        #加入历史帧数缓存
        # self.instance_queue = InstanceQueue(embed_dims=self.embed_dims, queue_length=10, tracking_threshold=0.5, feature_map_scale=(self.bev_h, self.bev_w))
        self.instance_queue = InstanceQueue(max_size=3)
        #加入简洁GNN
        self.temp_gnn = TemporalGNN(embed_dims=self.embed_dims)  
        #加入循环神经网络RNN
        # self.rnn_module = RNNModule(input_dim=self.embed_dims, output_dim=self.embed_dims)
        self.gnn_module = GNNModule(input_dim=self.embed_dims, output_dim=self.embed_dims)
    ...
    def forward(self,
                mlvl_feats,     #含来自上游网络的多层特征的元组，每个特征是形状为 (B, N, C, H, W) 的五维张量。
                img_metas,      #图像元数据，通常包含图像的尺寸、缩放信息等。
                prev_bev=None,  #上一时刻的 BEV 特征，用于持续处理。
                only_bev=False, # 仅计算 BEV 特征的标志。
                ego_his_trajs=None, # 自我历史轨迹，可能用于情景理解或预测。
                ego_lcf_feat=None,  # 自我相关特征，用于 ego 代理交互。
            ):
        """Forward function.
        Args:
        ...
        """
        # 获取批量大小和相机数量，并设定数据类型。接着从查询嵌入中获取相应的权重。
        # for i, feat in enumerate(mlvl_feats):
        #     print(f"mlvl_feats[{i}] shape:", feat.shape)
        # mlvl_feats[0] shape: torch.Size([4, 6, 256, 48, 80])
        # mlvl_feats[1] shape: torch.Size([4, 6, 256, 12, 20])
        
        bs, num_cam, _, _, _ = mlvl_feats[0].shape
        dtype = mlvl_feats[0].dtype
        object_query_embeds = self.query_embedding.weight.to(dtype)
        
        # 根据 map_query_embed_type 的值，对应地准备不同类型的地图查询嵌入。
        if self.map_query_embed_type == 'all_pts':
        ...
        # 解析变换器的输出，初始化输出类和轨迹的存储列表。
        bev_embed, hs, init_reference, inter_references, \
            map_hs, map_init_reference, map_inter_references = outputs

        hs = hs.permute(0, 2, 1, 3)

        current_feature = hs[-1]  # 当前要处理的特征（取最后一层）
        current_reference = init_reference  # 参考点
        self.instance_queue.push(current_feature, current_reference)  # Push current_feature 到队列中
        with torch.no_grad():
            # 获取缓存的历史特征并将其用于运动解码器
            his_feat, his_ref = self.instance_queue.get()  # Retrieve historical features from the queue
        
        # if len(his_feat) == 3:
        # print("current_feature Shape:", current_feature.shape)  # 检查 new_feat 的维度
        # print("his_feat Shape:", his_feat.shape)  # 检查 his_feat 的维度
        # print("his_ref Shape:", his_ref.shape)  # 检查 his_ref 的维度
        device = bev_queries.device
        current_feature = current_feature.to(device)
        his_feat = his_feat.to(device)
        combined_his_feat = self.gnn_module.aggregate(his_feat)  # 输出为 [B, A, D]
        
        # print("combined_his_feat Shape:", combined_his_feat.shape)  # 检查 new_feat 的维度

        new_feat = combined_his_feat + current_feature 
        # print("New Feature Shape:", new_feat.shape)  # 检查 new_feat 的维度
        # else:
        #     new_feat = current_feature
        ...
        #处理每个级别的 HS 输出
        for lvl in range(hs.shape[0]): # 使用 for 循环遍历隐藏状态 hs 的每一层
            if lvl == 0:
            ...
            # 通过相应的 cls_branches 和 reg_branches 来处理当前层的隐藏状态 hs[lvl]，分别生成类别分数 outputs_class 和回归输出 tmp。
            ...
            # 存储坐标：将处理后的坐标 (tmp[..., 0:2]) 进行克隆并附加到 outputs_coords_bev 列表中。这是为了在后续处理中保持坐标的独立副本。
        ...
        if self.motion_decoder is not None:
            ...
            batch_size, num_agent = outputs_coords_bev[-1].shape[:2] # 获取批量大小和代理数量
            # motion_query
            motion_query = new_feat.permute(1, 0, 2)
            # motion_query = hs[-1].permute(1, 0, 2)  # [A, B, D]     # 准备运动查询
            ...
            # 调用运动解码器，传入运动查询、位置编码和无效索引，得到运动的隐藏状态 motion_hs。
            ...
            raise NotImplementedError('Not implement yet')
        # 轨迹预测 使用轨迹分支对运动状态进行处理，得到最终的轨迹预测和轨迹类别。
        ...
        # planning 获取批量大小和代理数量
        (batch, num_agent) = motion_hs.shape[:2]
        #  处理自我历史特征 根据是否使用自我历史编码器，提取 ego 代理的历史特征。如果没有，则使用默认的权重作为特征。
        if self.ego_his_encoder is not None:
            ego_his_feats = self.ego_his_encoder(ego_his_trajs)  # [B, 1, dim]
        else:
            ego_his_feats = self.ego_query.weight.unsqueeze(0).repeat(batch, 1, 1)
        # 对 ego 代理和其他代理对象之间进行的解码器交互处理。
        # Interaction 定义 ego 查询与位置编码
        ...
        # ego <-> agent interaction 代理与 ego 的交互
        ...
        # ego <-> map interaction  ego 与地图的交互
        ...
        map_pos = map_outputs_coords_bev[-1]
        # use the most close pts pos in each map inst as the inst's pos 计算地图位置
        ...
        map_query, map_pos, map_mask = self.select_and_pad_query(  #选择和填充查询
            map_query, min_map_pos, map_conf,
            score_thresh=self.query_thresh, use_fix_pad=self.query_use_fix_pad
        )
        map_pos_emb = self.ego_map_pos_mlp(map_pos)
        ego_map_query = self.ego_map_decoder(   #ego 与地图查询的解码 最后调用 ego 地图解码器进行 ego 代理与地图之间的交互。
        ...
        # 根据不同的条件组合最终的 ego 特征，可能包括历史特征、代理查询和地图查询，形成多种情况的组合。
    ...
    def _map_get_target_single(self,
                           ...
                           gt_shifts_pts,
                           gt_bboxes_ignore=None):
        """"Compute regression and classification targets for one image. 用于计算单个图像的回归和分类目标
        Outputs from a single decoder layer of a single feature level are used.
        Args:
            cls_score (Tensor): Box score logits from a single decoder layer
                for one image. Shape [num_query, cls_out_channels].
        ...
        labels (Tensor): 每个图像的标签。
        label_weights (Tensor): 每个图像的标签权重。
        bbox_targets (Tensor): 每个图像的边界框目标。
        bbox_weights (Tensor): 每个图像的边界框权重。
        pts_targets (Tensor): 每个图像的关键点目标。
        pts_weights (Tensor): 每个图像的关键点权重。
        pos_inds (Tensor): 每个图像采样的正例索引。
        neg_inds (Tensor): 每个图像采样的负例索引。
        """
        num_bboxes = bbox_pred.size(0)   #
        # assigner and sampler #分配器和采样器
        gt_c = gt_bboxes.shape[-1]  
        # 使用 map_assigner 进行边界框分配，返回分配结果和顺序索引。
        assign_result, order_index = self.map_assigner.assign(bbox_pred, cls_score, pts_pred, # pts_pred 这是预测的关键点
                                             gt_bboxes, gt_labels, gt_shifts_pts,
                                             gt_bboxes_ignore)
        # 从分配结果中进行采样，获取正例和负例的索引。
        sampling_result = self.map_sampler.sample(assign_result, bbox_pred,
                                              gt_bboxes)
        pos_inds = sampling_result.pos_inds
        neg_inds = sampling_result.neg_inds
        # label targets  初始化标签张量，并将正例的标签赋值。
        labels = gt_bboxes.new_full((num_bboxes,),
                                    self.map_num_classes,
                                    dtype=torch.long)
        labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
        label_weights = gt_bboxes.new_ones(num_bboxes)
        # bbox targets 初始化边界框目标和权重，正例的权重设置为1。
        bbox_targets = torch.zeros_like(bbox_pred)[..., :gt_c]
        bbox_weights = torch.zeros_like(bbox_pred)
        bbox_weights[pos_inds] = 1.0 
        # pts targets  关键点目标 初始化关键点目标和权重，正例的权重设置为1。
    ...
    def map_get_targets(self,
        ...
        """"Compute regression and classification targets for a batch image.
        Outputs from a single decoder layer of a single feature level are used.
        函数用来为一批图像计算回归和分类目标，它整合了多个图像的预测结果和真实标签，并输出目标信息。
        ...
        gt_bboxes_ignore_list = [
            gt_bboxes_ignore_list for _ in range(num_imgs)
        ]  # 获取输入列表中的图像数量，并将 gt_bboxes_ignore_list 标准化以确保它与图像数量相同。
