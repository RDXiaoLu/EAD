...
class VADHead(DETRHead):
    ...
    def forward(self,
        ...
        if self.motion_decoder is not None:
            ...
            # collision_boxes = motion_hs[..., :6]  # 假设 motion_hs 的前6个维度就是边界框信息  
            # other_boxes = outputs_coords[-1]  # 按您的模型结构选择合适的输出作为其他智能体的框 
            
            # print("Collision boxes shape:", collision_boxes.shape)  
            # print("Other boxes shape:", other_boxes.shape)  
            
            # detected_collisions = []  # 用于存储碰撞状态  
            # for i in range(batch_size):  
            #     for j in range(num_agent):   
            #         collision_result = check_collision(collision_boxes[i, j].unsqueeze(0), other_boxes[i])  
            #         detected_collisions.append(collision_result)  

            # # 碰撞避免处理  
            # detected_collisions = torch.tensor(detected_collisions).reshape(batch_size, num_agent)  # 变成合适的形状  
            # score_offset = detected_collisions.float() * -999  # 对发生碰撞的部分给予极低的分数  
            # outputs_classes[-1] += score_offset.unsqueeze(-1)  # 更新类别分数
        else:
            raise NotImplementedError('Not implement yet')
