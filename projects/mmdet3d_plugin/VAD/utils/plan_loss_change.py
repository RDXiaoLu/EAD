...
class PlanCollisionLoss(nn.Module):
    ...
    Args:
        reduction (str, optional): The method to reduce the loss.
            Options are "none", "mean" and "sum".
        loss_weight (float, optional): The weight of loss.
        agent_thresh (float, optional): confidence threshold to filter agent predictions. 用于过滤智能体预测的置信度阈值，低于该阈值的预测将被忽略。
        x_dis_thresh (float, optional): distance threshold between ego and other agents in x-axis. 自我车辆与其他智能体在 X 和 Y 轴上的距离阈值，防止碰撞。
    ...
    def forward(self,
                ego_fut_preds,   # 自我车辆未来可能位置的预测
                agent_preds,     # 当前智能体位置的预测。
                agent_fut_preds, # 智能体未来可能位置的预测。
                agent_score_preds, # 智能体的置信度预测，反映智能体预测的置信程度。
                agent_fut_cls_preds, #  智能体未来轨迹的类别预测（例如，是否是车辆，行人等）。
                weight=None,
                avg_factor=None,
                reduction_override=None):
        """Forward function.
...
def plan_col_loss(
    ...
    #对 pred 和 agent_fut_preds 执行累加操作，表示把之前的预测累加到当前时间步，以生成更完整的轨迹。
    pred = pred.cumsum(dim=-2)
    agent_fut_preds = agent_fut_preds.cumsum(dim=-2)
    #目标位置是通过为每个智能体的当前位置加上其未来的预测结果而得到的。这意味着我们将每个智能体可能的未来位置与当前目标结合起来，形成一个完整的轨迹。
    target = target[:, :, None, :] + agent_fut_preds  
    # filter distant agents from ego vehicle 过滤远离的智能体
    ...
    # [B, fut_ts]
    # X 轴损失 对于 X 轴，判断距离是否超过阈值，若超过则损失为0，否则计算距离和阈值的差值作为损失
    ...
    #更严格的约束
    # Calculate overall distance using Pythagorean theorem  
    overall_dist = torch.sqrt(x_min_dist**2 + y_min_dist**2)
    # Calculate collision loss using combined loss  
    collision_loss = overall_dist
    
    safe_idx = collision_loss > torch.sqrt(torch.as_tensor(x_dis_thresh)**2 + torch.as_tensor(y_dis_thresh)**2)
    unsafe_idx = collision_loss <= torch.sqrt(torch.as_tensor(x_dis_thresh)**2 + torch.as_tensor(y_dis_thresh)**2)
    x_loss[unsafe_idx] = x_dis_thresh - x_loss[unsafe_idx]
    y_loss[unsafe_idx] = y_dis_thresh - y_loss[unsafe_idx]
