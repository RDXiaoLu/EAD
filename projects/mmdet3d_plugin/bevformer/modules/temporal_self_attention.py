...
class TemporalSparseAttention(BaseModule):  
    """Enhanced spatio-temporal attention with adaptive scale awareness"""  
    def __init__(self,  
                 embed_dims=256,  
                 num_heads=8,  
                 num_levels=4,  
        ...
        # 引入稀疏注意力组件  
        self.gen_tau = nn.Linear(embed_dims*num_bev_queue, num_heads)  
    ...
    def forward(self,  
        ...
        if value is None:  
            value = torch.stack([query, query], 1).reshape(bs*2, num_query, self.embed_dims)  
        
        value = self.value_proj(value)  
        value = value.reshape(bs*self.num_bev_queue, num_query, self.num_heads, -1)  
        
        # 生成自适应注意力参数  
        tau = self.gen_tau(query)  # [B, Q, H]  
        spatial_attn_mask = self._calc_spatial_mask(query_bbox, tau)  # [B*H, Q, Q]  
        ...
        if reference_points.shape[-1] == 2:
            ...
            raise ValueError(
                ...
                f' 2 or 4, but get {reference_points.shape[-1]} instead.')
        # 应用空间约束  
        attention_weights = attention_weights * spatial_attn_mask.view(  
            bs, num_query, self.num_heads, 1, 1, 1)  
    ...
    def _calc_spatial_mask(self, bboxes, tau):  
        """空间注意力掩码生成"""  
        centers = decode_bbox(bboxes, self.pc_range)[..., :2]  # [B, Q, 2]  
        
        dist_matrix = []  
        for b in range(centers.shape[0]):  
            dist = torch.norm(centers[b].unsqueeze(1) - centers[b].unsqueeze(0), dim=-1)  
            dist_matrix.append(dist.unsqueeze(0))  
        
        dist_matrix = torch.cat(dist_matrix, 0)  # [B, Q, Q]  
        spatial_mask = -dist_matrix.unsqueeze(1)  # [B, 1, Q, Q]  
        
        # 加入自适应缩放  
        spatial_mask = spatial_mask * tau.permute(0, 2, 1).unsqueeze(-1)  # [B, H, Q, Q]  
        return spatial_mask.flatten(0, 1)  # [B*H, Q, Q]